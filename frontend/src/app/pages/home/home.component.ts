...
export class HomeComponent implements OnInit {
  // texts content
  textContent: IKojiConfigGeneric = {};
  // images content
  imageContent: string;
  ...
  init(): void {
    ...
    this.setImageContent(this.kojiService.getEditor('theme'));
  }
    ...
    if (contentProps.length) {
      ...
      contentProps.map((prop) => {
        ...
        if (prop && prop.startsWith(APP.paths.home)) {
          this.textContent[prop] = textContent[prop];
        }
      });
    }
  }
  ...
  /**
   * Sets page image content
   * @param theme IKojiConfigGeneric
   */
  private setImageContent(theme: IKojiConfigGeneric): void {
    this.imageContent = theme.home_main_image || APP.defaults.images.home;
  }
